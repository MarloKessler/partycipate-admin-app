...
function SurveyOverview() {
    ...
    return (
        <OverviewView onFilter={handleFilter}/>
    )
}


export default function OverviewView({ containerClassName="", title, helpSection, className="" }) {
    ...
    return (
        <StandardPage className={`overview-view ${containerClassName}`} helpSection={helpSection} title={title}>    
...
function Surveys({surveys, onFilter}) {
    ...
    const handleSearchInput = event => {
        ...
        if (searchInput === "") setFilteredSurveys(surveys)
            ...
            const filteredArray = surveys.filter(survey => onFilter(survey, searchInput))
    ...
    return (
        <div>
            <div className="so-search-bar-container">
                <CardElement className="so-search-bar"><input type="text" placeholder="xyz"></input></CardElement>
            </div>
